...
pub enum X87Register
{
	...
	/// Register `ST(1)`.
	///
	/// Contains the second `long double` floating-point return value from a function call when using the System V Application Binary Interface (ABI) for AMD64; typically used for returning values of type `complex long double`, where register `ST(0)` contains the 'real' part as a `long double` and `ST(1)` contains the 'imaginary' part as a `long double`.
	///
	/// A scratch register (also known as a temporary or (function) caller-saved register) when using the System V Application Binary Interface (ABI) for AMD64.
	ST1 = 1,
...
impl X87Register
{
	...
	/// Contains the second `long double` floating-point return value from a function call when using the System V Application Binary Interface (ABI) for AMD64.
